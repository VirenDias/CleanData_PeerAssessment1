...
This file contains information about the data contained within `tidy_data.txt`.

## Identifiers

* `subject` - The ID of the test subject. An integer between `1` and `30`.
* `activitiy` - The activity being performed when the measurements were taken. Can be one of 6 activties:
    + `WALKING`
    + `WALKING_UPSTAIRS`
    + `WALKING_DOWNSTAIRS`
    + `SITTING`
...
Each value in the `tidy_data.txt` dataset represents the average value of each variable for each activity and each subject.